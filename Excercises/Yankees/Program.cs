    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            Dictionary<int, RetiredPlayer> retiredYankees = new Dictionary<int, RetiredPlayer>()
            {
                ...
                {44, new RetiredPlayer("Reggi Jackson", 1993) },
            };

            foreach (int jerseyNumber in retiredYankees.Keys)
            {
                Console.WriteLine($"#{jerseyNumber} {retiredYankees[jerseyNumber].Name} retired in {retiredYankees[jerseyNumber].YearRetired}");
