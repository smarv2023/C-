/*
    These are only samples from Chapter 5 book of 
    C# 11 and .NET 7 – Modern Cross-Platform Development Fundamentals - Seventh Edition
 */
using Packt.Shared;
// Change the currency to Great Britain
...
// Different string ways of interpolation
WriteLine(format: "{0} was born on {1:dddd, d MMMM yyyy}", arg0:bob.Name, arg1:bob.DateOfBirth);
WriteLine(String.Format("{0}'s favorite wonder is {1}. Its integer is {2}", bob.Name, bob.FavoriteAncientWonder, (int)bob.FavoriteAncientWonder));
...
// Codes of instance and invoking class methods
WriteLine();
Person alice = new()
{
    Name = "Alice Jones",
...
WriteLine(format: "{0} of {1} was created at {2:hh:mm:ss}", arg0: blankPerson.Name, blankPerson.HomePlanet, blankPerson.Instantiated);
...
WriteLine($"Thre are {fruitNamed2.Number} {fruitNamed2.Name}.");

(string Name, int Number) fruitNamed3 = ("Banana", 5);
WriteLine($"There are {fruitNamed3.Number} {fruitNamed3.Name}.");

//(string name1, DateTime dob1) = bob;
var (name1, dob1) = bob;
WriteLine($"Deconstructed: {name1}, {dob1}");

var (name2, dob2, fav2) = bob;
WriteLine($"Deconstructed: {name1}, {dob1}, {fav2}");

WriteLine(bob.SayHello());
WriteLine(bob.SayHello("Emily"));

// Invoking Optional Parameter
WriteLine();
WriteLine(bob.OptionalParameters());
WriteLine(bob.OptionalParameters("Jump!", 0.01));
WriteLine(bob.OptionalParameters("Jump!", 0.01, false));
WriteLine(bob.OptionalParameters(number: 52.7, command: "Hide!"));
WriteLine(bob.OptionalParameters(command: "Seek!", active: false));

// ref, out in parameter and argument
WriteLine();
int a = 10; // this will not change because the value inside the method will stay in there
int b = 20; // this will be use as a reference "ref" and return the new value after processing inside the method
int c = 30; // this will automatically replace by the inside value "out"
...
int d = 10;
int e = 20;
// f will just be declaire inside the argument
WriteLine($"Before: d = {d}, e = {e}, f doesn't exist yet!");
// simplified C# 7.0 or later syntax for the out parameter 
bob.PassingParameters(d, ref e, out int f);
WriteLine($"After: d = {d}, e = {e}, f = {f}");

// sam partial Person PersonAutoGen.cs
WriteLine();
Person sam = new()
{
    Name = "Sam",
...
sam.Children.Add(new() { Name = "Chalie", DateOfBirth = new(2010, 3, 18) });
...
// Local function example PersonAutoGen factorial
WriteLine();
WriteLine("Factorial example Local function");
int number = 5;
try
{
    WriteLine($"{number}!");
